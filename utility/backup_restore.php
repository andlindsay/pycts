...
<div id="backup_restore">
	...
	<div class="section">
		<h2>Create Backup</h2>
		<p>Click the button below to download a backup. No data will be changed or deleted.</p>
	...
	<div class="section">
		...
		<p>
			<strong>Warning:</strong> the restore process causes all students and credits in the database to be <strong>deleted</strong>. After that 
			happens, all students in the CSV file are added, and miscellaneous credits are added to each to restore correct credit counts. If you 
			are certain this is what you want to do, choose the CSV backup file to upload, then type the word "confirm" in the box.
		</p>
		
		<form enctype="multipart/form-data" action="user.php?backup_restore" method="post">
			<p><input type="hidden" name="action_timestamp" value="$tstamp"/></p>
			<dl>
				<dt>CSV Backup File</dt>
				<dd><input name="uploaded" type="file"/></dd>
				<dt>Confirmation</dt>
				<dd><input type="text" name="confirm"/></dd>
			</dl>
			<p><input type="submit" name="restore" value="Restore From Backup"/></p>
		</form>
	</div>
	...
	<div class="section">
		<h2>Database Wipe</h2>
		<p>
			This action will forcibly delete all students and their associated credits. Please consider <a href="user.php?backup_restore">backing up</a> 
			this data before it is removed. If you truly want to do this, type the word "confirm" into the text box below, and hit the button.
		</p>
		<form action="user.php?backup_restore" method="post">
			<input type="hidden" name="action_timestamp" value="$tstamp"/>
			<p>
